...
namespace AssetInventory
{
    public partial class IndexUI
    {
        ...
        private void DrawAssetManager()
        {
            ...
            AssetInventory.Config.indexAssetManager = GUILayout.Toggle(AssetInventory.Config.indexAssetManager, "Unity Asset Manager", GUILayout.MaxWidth(250));
            ...
            if (AssetInventory.Config.indexAssetManager)
            {
                ...
                EditorGUILayout.HelpBox("In order to connect to Asset Manager, the following packages need to be installed: Unity Cloud Assets 1.3.0+, Unity Cloud Identity 1.2.0+", MessageType.Error);
                if (GUILayout.Button("Install Packages"))
                {
                    Client.AddAndRemove(new[] {"com.unity.cloud.assets@1.3.0", "com.unity.cloud.identity@1.2.0"});
                }
#else
        ...
        {
            CloudAssetManagement cam = await AssetInventory.GetCloudAssetManagement();
        ...
        private async void DeleteCollection(AssetInfo info)
        {
            ...
            AssetInventory.RemovePackage(info, true);
            AssetInventory.TriggerPackageRefresh();
        }
        ...
        private async void AddAssetsToProject(AssetInfo project, List<AssetInfo> assets)
        {
            ...
            foreach (AssetInfo info in assets)
            {
                ...
                if (folderMode)
                {
                    ...
                    await AssetInventory.CopyTo(info, path, true, true, false, true);
                ...
                {
                    path = await AssetInventory.EnsureMaterializedAsset(info.ToAsset(), info);
                }
                if (path == null)
                {
