...
namespace AssetInventory
{
    public sealed class PreviewWizardUI : BasicEditorUI
    {
        ...
        {
            string assetFilter = PreviewImporter.GetAssetFilter(_assets);
            string countQuery = "select count(*) from AssetFile";

            _totalFiles = DBAdapter.DB.ExecuteScalar<int>($"{countQuery} {BASE_JOIN} {assetFilter}");
            _imageFiles = DBAdapter.DB.ExecuteScalar<int>($"{countQuery} {BASE_JOIN} {assetFilter} and AssetFile.Type in ('" + string.Join("','", AssetInventory.TypeGroups["Images"]) + "')");
        ...
        public override void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            GUILabelWithText($"{UIStyles.INDENT}Pre-Provided", $"{_providedFiles:N0}", labelWidth);
            ...
            EditorGUILayout.BeginHorizontal();
            GUILabelWithText($"{UIStyles.INDENT}Recreated", $"{_recreatedFiles:N0}", labelWidth);
            ...
            EditorGUILayout.BeginHorizontal();
            GUILabelWithText($"{UIStyles.INDENT}Erroneous", $"{_erroneousFiles:N0}", labelWidth);
            ...
            EditorGUILayout.BeginHorizontal();
            GUILabelWithText($"{UIStyles.INDENT}Not Applicable", $"{_noPrevFiles:N0}", labelWidth, "These are files for which no previews are created, e.g. documents, scripts, controllers. Only a generic icon will be shown.");
            ...
            {
                Schedule("and AssetFile.Type in ('" + string.Join("','", AssetInventory.TypeGroups["Images"]) + "')");
            }
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.EndHorizontal();
            ...
            if (_showAdv)
            {
                ...
                {
                    string path = AssetInventory.GetPreviewFolder();
                    if (_assets != null && _assets.Count == 1)
                    {
                        path = _assets[0].GetPreviewFolder(AssetInventory.GetPreviewFolder());
                    }
        ...
        private async void RestorePreviews()
        {
            ...
            int restored = await new PreviewImporter().RestorePreviews(_assets);
        ...
        private async void RecreatePreviews()
        {
            ...
            int created = await new PreviewImporter().RecreateScheduledPreviews(_assets, _allAssets);
