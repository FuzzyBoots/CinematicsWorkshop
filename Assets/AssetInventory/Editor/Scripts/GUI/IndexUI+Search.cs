...
namespace AssetInventory
{
    public partial class IndexUI
    {
            ...
            else
            {
                    ...
                    if (GUILayout.Button(UIStyles.Content("?", "Show example searches"), GUILayout.Width(20)))
                    {
                        ...
                        searchUI.Init((searchPhrase, searchType) =>
                        {
                            ...
                            else
                            {
                                ...
                                if (typeIdx >= 0) AssetInventory.Config.searchType = typeIdx;
                            }
                ...
                if (AssetInventory.Config.showSearchFilterBar)
                {
                    ...
                    if (AssetInventory.Config.showDetailFilters)
                    {
                        ...
                        {
                            GUILayout.BeginHorizontal();
                            EditorGUILayout.LabelField("Image Type", EditorStyles.boldLabel, GUILayout.Width(85));
                ...
                if (_sgrid.contents != null && _sgrid.contents.Length > 0)
                {
                    ...
                    if (EditorGUI.EndChangeCheck() || (_allowLogic && _searchDone))
                    {
                        ...
                        if (_selectedEntry != null)
                        {
                            ...
                            if (AssetInventory.Config.autoCalculateDependencies == 1)
                            {
                ...
                if (!hideDetailsPane && AssetInventory.Config.showSearchDetailsBar)
                {
                    ...
                    if (_sgrid.selectionCount <= 1)
                    {
                        ...
                        else
                        {
                            ...
                            if (_selectedEntry.Downloaded)
                            {
                                ...
                                if (_selectedEntry.AssetSource == Asset.Source.AssetManager || AssetInventory.NeedsDependencyScan(_selectedEntry.Type))
                                {
                                ...
                                if (!searchMode)
                                {
                                    ...
                                    EditorGUI.BeginDisabledGroup(_previewInProgress);
                                    if ((ShowAdvanced() || _selectedEntry.PreviewState == AssetFile.PreviewOptions.Error || _selectedEntry.PreviewState == AssetFile.PreviewOptions.None || _selectedEntry.PreviewState == AssetFile.PreviewOptions.Redo) && GUILayout.Button("Recreate Preview"))
                                    {
                                        RecreatePreview(_selectedEntry);
                                    }
                                    EditorGUI.EndDisabledGroup();

#if USE_ASSET_MANAGER && USE_CLOUD_IDENTITY
                                    if (AssetInventory.Config.indexAssetManager)
                                    {
                                ...
                                UIBlock("asset.actions.tag", () =>
                                {
                                    ...
                                    if (_selectedEntry.AssetTags != null && _selectedEntry.AssetTags.Count > 0)
                                    {
                                        ...
                                        foreach (TagInfo tagInfo in _selectedEntry.AssetTags)
                                        {
                                            ...
                                            {
                                                AssetInventory.RemoveTagAssignment(_selectedEntry, tagInfo, true, true);
                    ...
                    else
                    {
                        ...
                        UIBlock("asset.bulk.actions.tag", () =>
                        {
                            ...
                            foreach (KeyValuePair<string, Tuple<int, Color>> bulkTag in _assetFileBulkTags)
                            {
                                ...
                                {
                                    AssetInventory.RemoveAssetTagAssignment(_sgrid.selectionItems, bulkTag.Key, true);
                    ...
                    if (_showSettings)
                    {
                        ...
                        AssetInventory.Config.searchSubPackages = EditorGUILayout.Toggle(AssetInventory.Config.searchSubPackages);
                        GUILayout.EndHorizontal();
                        if (EditorGUI.EndChangeCheck())
                        {
                            dirty = true;
                        ...
                        AssetInventory.Config.pingImported = EditorGUILayout.Toggle(AssetInventory.Config.pingImported);
                        GUILayout.EndHorizontal();
                        ...
                        AssetInventory.Config.autoCalculateDependencies = EditorGUILayout.Popup(AssetInventory.Config.autoCalculateDependencies, _dependencyOptions);
                        GUILayout.EndHorizontal();
        ...
        private async void CopyTo(AssetInfo info, string targetFolder, bool withDependencies = false, bool withScripts = false, bool autoPing = true, bool fromDragDrop = false, bool reimport = false)
        {
            ...
            string mainFile = await AssetInventory.CopyTo(info, targetFolder, withDependencies, withScripts, fromDragDrop, false, reimport);
            if (autoPing && mainFile != null)
            {
                if (AssetInventory.Config.pingImported) PingAsset(new AssetInfo().WithProjectPath(mainFile));
        ...
        private bool IsFilterApplicable(string filterName)
        {
            ...
            if (AssetInventory.FilterRestriction.TryGetValue(filterName, out string[] restrictions))
            {
            ...
            if (!AssetInventory.Config.filterOnlyIfBarVisible || AssetInventory.Config.showSearchFilterBar)
            {
                ...
                if (_selectedAsset > 0 && _assetNames.Length > _selectedAsset)
                {
                    ...
                    if (asset.LastIndexOf('[') > 0)
                    {
                        ...
                        if (AssetInventory.Config.searchSubPackages)
                        {
        ...
        private async void LoadTextures(bool firstPageOnly, CancellationToken ct)
        {
            ...
            foreach (AssetInfo info in files)
            {
                ...
                if (info.HasPreview(true)) AssetImporter.ValidatePreviewFile(info, previewFolder);
        ...
        private void OpenInSearch(AssetInfo info, bool force = false)
        {
            ...
            _selectedAsset = Array.IndexOf(_assetNames, _assetNames.FirstOrDefault(a => a == info.GetDisplayName() + $" [{info.AssetId}]"));
            if (_selectedAsset < 0) _selectedAsset = Array.IndexOf(_assetNames, _assetNames.FirstOrDefault(a => a == info.GetDisplayName().Substring(0, 1) + "/" + info.GetDisplayName() + $" [{info.AssetId}]"));
            if (_selectedAsset < 0) _selectedAsset = Array.IndexOf(_assetNames, _assetNames.FirstOrDefault(a => a.EndsWith(info.GetDisplayName() + $" [{info.AssetId}]")));
        ...
        private async void PerformCopyToBulk(List<AssetInfo> infos, string targetPath)
        {
            ...
            if (AssetInventory.Config.pingImported) PingAsset(infos[0]);
        }
