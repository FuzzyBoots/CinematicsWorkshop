...
namespace AssetInventory
{
    public sealed class WrongDimensionPreviewFilesValidator : Validator
    {
        ...
        private static async Task<List<AssetInfo>> CheckPreviewDimensions()
        {
            ...
            string typeStr = string.Join("\",\"", AssetInventory.TypeGroups["Images"]);
            string query = "select * from AssetFile where (PreviewState = ? or PreviewState = ?) and Type in (\"" + typeStr + "\") and (Width >= ? or Height >= ?)";
            List<AssetInfo> files = DBAdapter.DB.Query<AssetInfo>(query, AssetFile.PreviewOptions.Provided, AssetFile.PreviewOptions.Custom, AssetInventory.Config.upscaleSize, AssetInventory.Config.upscaleSize).ToList();
            ...
            foreach (AssetInfo file in files)
            {
                ...
                if (dimensions.Item1 != AssetInventory.Config.upscaleSize && dimensions.Item2 != AssetInventory.Config.upscaleSize)
                {
                    result.Add(file);
                }
            }
