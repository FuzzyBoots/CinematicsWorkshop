...
namespace AssetInventory
{
    public class CloudAssetManagement
    {
        ...
        {
            _busyCount++;
            DatasetCreation datasetCreation = new DatasetCreation(name)
            {
                Description = description
            };
        ...
        public async Task SetTags(Asset asset, AssetFile assetFile, List<string> tags)
        {
            _busyCount++;

            IAssetProject project = await SelectProjectAsync(asset);
            IAsset cloudAsset = await project.GetAssetAsync(new AssetId(assetFile.Guid), new Unity.Cloud.Common.AssetVersion(assetFile.FileVersion), CancellationToken.None);
            //IAsset cloudAsset = await project.GetAssetAsync(new AssetId(assetFile.Guid), CancellationToken.None);
            if (cloudAsset == null)
            {
                _busyCount--;
                return;
            ...
            try
            {
                await cloudAsset.UpdateAsync(assetUpdate, CancellationToken.None);
            }
            catch (Exception e)
            {
                Debug.LogError($"Failed to set tags: {e.Message}");
            }
        ...
        public async Task<List<string>> FetchAssetFromRemote(Asset asset, AssetFile assetFile, string targetFolder)
        {
            ...
            // TODO: simplify this by constructing the AssetDescriptor somehow to jump directly to the asset
            IAssetProject project = await SelectProjectAsync(asset);
            IAsset cloudAsset = await project.GetAssetAsync(new AssetId(assetFile.Guid), CancellationToken.None);
            if (cloudAsset == null)
            {
