    ...
    public static class AssetStore
    {
        ...
        public static async Task<AssetPurchases> RetrievePurchases()
        {
            ...
            if (result != null && result.total > PAGE_SIZE)
            {
                ...
                for (int i = 1; i <= pageCount; i += AssetInventory.Config.maxConcurrentUnityRequests)
                {
                    List<Task<AssetPurchases>> currentBatch = new List<Task<AssetPurchases>>();

                    for (int j = i; j < i + AssetInventory.Config.maxConcurrentUnityRequests && j <= pageCount; j++)
                    {
                    ...
                    {
                        AssetInventory.MainProgress = i + k + 1;
